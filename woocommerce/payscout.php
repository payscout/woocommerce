...
/*
	...
	*/
		...
		if (curl_error($curl)) {
			throw new Exception( __( 'CURL ERROR: '.curl_errno($curl) . '::' . curl_error($curl), 'woocommerce' ) ); 
		...
		{			
...

			{
				...
				$wc_order->add_order_note( __( $content['status']. ' on '.date("d-m-Y h:i:s e").' with Transaction ID = '.$content['status'].' using payscout gateway, cardholder authentication verification response code: '.$content['status'], 'woocommerce' ) );
